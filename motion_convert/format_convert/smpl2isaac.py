...
def convert2isaac(data):
    ...
    pose_aa_mj = pose_aa[..., smpl_2_isaac, :].clone()

    motion_length = pose_aa.shape[0]

    pose_quat = sRot.from_rotvec(pose_aa_mj.reshape(-1, 3)).as_quat().reshape(motion_length, 24, 4)
    pose_quat = torch.Tensor(pose_quat)

    with open('./smpl_skeleton_tree.pkl', 'rb') as f:
    ...
    pose_quat = rotated_sk_state.local_rotation.numpy()
    pose_quat_global = rotated_sk_state.global_rotation.numpy()
    root_trans_offset = rotated_sk_state.root_translation.numpy()
    pose_aa = sRot.from_quat(pose_quat.reshape(-1,4)).as_rotvec().reshape(-1,24,3)
    ...
    new_motion_out['beta'] = np.zeros(10)
