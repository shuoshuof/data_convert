...
class Draw3DStateDICT(BasePlotterTask):
    _lines_task: Draw3DLines  # sub-task for drawing lines
    _dots_task: Draw3DDots  # sub-task for drawing dots
    ...
    def __init__(
        ...
        super().__init__(task_name=task_name, task_type="3DStateDICT")
        lines, dots = Draw3DStateDICT._get_lines_and_dots(skeleton_state)
        self._lines_task = Draw3DLines(
            self.get_scoped_name("bodies"), lines, joints_color, alpha=alpha
        )
        self._dots_task = Draw3DDots(
    ...
    def update(self, skeleton_state) -> None:
        self._update(*Draw3DStateDICT._get_lines_and_dots(skeleton_state))
...
from poselib.poselib.skeleton.skeleton3d import MotionDICT

class Draw3DMotionDICT(BasePlotterTask):
    def __init__(
        self,
        task_name: str,
        skeleton_motion,
        ...
        super().__init__(task_name=task_name, task_type="3DMotionDICT")
        self._trail_length = trail_length
        
        skeleton_motion = MotionDICT(skeleton_motion.global_translation, skeleton_motion.skeleton_tree)
        ...
        if frame_index is not None:
            curr_skeleton_motion = self._skeleton_motion[frame_index]
        self._skeleton_state_task = Draw3DStateDICT(
            self.get_scoped_name("skeleton_state"),
            curr_skeleton_motion,
            joints_color=joints_color,
            lines_color=lines_color,
    ...
    def update(self, frame_index=None, reset_trail=False, skeleton_motion=None) -> None:
        ...
        curr_skeleton_motion = self._skeleton_motion.clone()
        if frame_index is not None:
            curr_skeleton_motion = curr_skeleton_motion[frame_index]
        # if reset_trail:
        #     self._com_pos = curr_skeleton_motion.root_translation.numpy()[
        #         np.newaxis, ...
        #     ].repeat(self._trail_length, axis=0)
