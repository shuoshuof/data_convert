...
class Matplotlib3DPlotterAx(BasePlotter):
    _fig: plt.figure  # plt figure
    # _ax: p3.Axes3D  # plt 3d axis
    # stores artist objects for each task (task name as the key)
    _artist_cache: Dict[str, Any]
    ...
    def __init__(self, fig, ax, task: "BasePlotterTask", boundary=False) -> None:
        self._fig = fig #plt.figure()
        self._ax = ax #p3.Axes3D(self._fig)
        self._artist_cache = {}
        ...
        self._boundaries = {
            'min': task._skeleton_motion.global_translation.numpy().min(axis=0).min(axis=0),
            'max': task._skeleton_motion.global_translation.numpy().max(axis=0).max(axis=0),
        } if boundary else None

        self._init_lim()
        super().__init__(task)

        # for tmp in self._task_primitives:
        #     if tmp.task_type == "Draw3DDots":
        #         print(tmp[:].shape)
        # task._skeleton_motion.global_translation.shape
    ...
    def _init_lim(self):
        ...
        if self._boundaries is not None: 
            self._curr_x_min = self._boundaries['min'][0]
            self._curr_y_min = self._boundaries['min'][2]
            self._curr_z_min = self._boundaries['min'][1]
            self._curr_x_max = self._boundaries['max'][0]
            self._curr_y_max = self._boundaries['max'][2]
            self._curr_z_max = self._boundaries['max'][1]
